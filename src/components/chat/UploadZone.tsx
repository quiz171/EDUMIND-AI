...
import { UploadCloud, FileText, CheckCircle2, AlertCircle, Loader2, X, Paperclip } from 'lucide-react';
    ...
    return (
      <div className="relative">
        ...
        <button
          ...
          className="p-1.5 sm:px-2.5 sm:py-1.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-stone-300 hover:text-emerald-400 transition-colors cursor-pointer flex items-center gap-1.5 text-xs disabled:opacity-50"
          title="Upload notes (PDF/DOCX) or past question photo"
        >
          {isUploading ? (
          ...
          <span className="hidden sm:inline font-medium">Upload / Past Question</span>
        </button>
        {errorMsg && (
          <div className="absolute bottom-full left-0 mb-2 w-64 p-2 bg-rose-950/90 border border-rose-800 text-rose-300 text-[11px] rounded-xl shadow-xl z-30">
            {errorMsg}
          </div>
        )}
      </div>
